...
public static class AzureStorageEmulatorResourceBuilderExtensions {
    public static IResourceBuilder<AzureStorageEmulatorResource> WithCopyFromAzureCommand(
        ...
        builder.WithCommand(
            name: "copy-blobs-from-azure",
                ...
                executeCommand: OnRunCopyBlobsFromAzureCommandAsync,
                commandOptions: commandOptions);

        return builder;
    }

    private static async Task<ExecuteCommandResult> OnRunCopyBlobsFromAzureCommandAsync(ExecuteCommandContext context) {
        var logger = context.ServiceProvider.GetRequiredService<ILogger<Program>>();

        logger.LogInformation("Copying blobs from Azure Storage to Azurite...");

        var storageAccount = "umbraco-aspire-storage-blobs";
        var containerName = "umbraco-aspire";
        var azuritePort = 60093;

        try {
            await CopyBlobsToAzuriteAsync(context, storageAccount, containerName, azuritePort);
        } catch(Exception ex) {
    ...
    private static async Task CopyBlobsToAzuriteAsync(ExecuteCommandContext context, string storageAccount, string containerName, int azuritePort) {
        var logger = context.ServiceProvider.GetRequiredService<ILogger<Program>>();
        ...
        // For Umbraco we can pass prefix: "media/" to only copy media blobs
        await Parallel.ForEachAsync(blobContainerClient.GetBlobsAsync(prefix: "media/", cancellationToken: cancellationToken), async (blob, cancellationToken) => {
...
public sealed class AzuriteConstants {
